...
export const up = (pgm: MigrationBuilder) => {
  ...
  pgm.sql(buildBusinessContactFromBusinessContactsAndPreferedContactMethods);
...
export const down = (pgm: MigrationBuilder) => {
  ...
  const buildPreferedContactMethodsFromBusinessContact = `
    WITH prepared_view AS (
      ...
      WHERE form_establishments.siret = prepared_view.siret
  `;
  pgm.sql(buildPreferedContactMethodsFromBusinessContact);
  ...
  WITH prepared_view AS 
    ...
    WHERE form_establishments.siret = prepared_view.siret
`;
  pgm.sql(buildBusinessContactsFromBusinessContact);

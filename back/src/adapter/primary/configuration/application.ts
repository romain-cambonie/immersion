import { ApplicationConfiguration, EnvironmentTypes } from "../../../ports/ApplicationConfiguration"

export const applicationConfigurationFromEnvProcess = (): ApplicationConfiguration => {
  return {
...
export const isTruthy = <T extends string>(element: T | undefined): element is T => !!element;
...
export const throwIfNotDefined =
  (variableName: string): string => {
    const value = process.env[variableName]?.trim();
    if (!isTruthy(value)) throw new Error(`Expected ${variableName} environment variable to be defined and not empty`);
    return value;
  };
